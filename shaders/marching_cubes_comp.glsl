...
void main() {
    ...
    for (int i = 0; triangulation[i] != -1; i += 3) {
        ...
        float valA = (levels[a0] + levels[b0]);
        float valB = (levels[a1] + levels[b1]);
        float valC = (levels[a2] + levels[b2]);

        uint baseIndex = atomicAdd(floatCount, 3 * 7);

        Vertex vertexA = Vertex(posA, vec3(1.0), float(baseIndex));
        Vertex vertexB = Vertex(posB, vec3(1.0), float(baseIndex + 7));
        Vertex vertexC = Vertex(posC, vec3(1.0), float(baseIndex + 14));
        ...
        addVertex(vertexB, baseIndex + 7);
        addVertex(vertexA, baseIndex + 14);
    }
}
